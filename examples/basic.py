...
def structured_logging_example():
    ...
    log.info("User login", user="alice", role="admin", success=True, login_time=time.time())
    log.info("Database query", table="users", query_time_ms=45.2, rows_affected=1)
    log.error("Payment failed", user="bob", amount=99.99, currency="USD", error_code="INSUFFICIENT_FUNDS")
...
def exception_example():
    ...
    log = get_logger("exception_example", json=False)  # Human-readable for better traceback display
    ...
    try:
        ...
        log.exception("Mathematical error occurred", operation="division", dividend=10, divisor=0)
    ...
    try:
        ...
        log.exception("Missing data field", required_field=str(e), available_fields=list(data.keys()))
    ...
    try:
        ...
        log.exception("Data type error in calculation", data_types=[type(x).__name__ for x in numbers])
