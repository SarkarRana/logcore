...
class UserService:
    ...
    async def get_user(self, user_id: int, correlation_id: str) -> Optional[User]:
        ...
        with user_service_log.with_correlation_id(correlation_id):
            ...
            with user_service_log.time("user_lookup", user_id=user_id):
                ...
                if user:
                    user_service_log.info("User found", 
                                        user_id=user_id, 
                                        user_name=user.name,
                                        user_tier=user.tier)
                else:
    ...
    async def update_user_activity(self, user_id: int, activity: str, correlation_id: str):
        """Update user activity log."""
        with user_service_log.with_correlation_id(correlation_id):
            user_service_log.info("Updating user activity",
                                user_id=user_id,
                                activity=activity,
                                timestamp=time.time())
...
class OrderService:
    ...
    async def create_order(self, user_id: int, items: List[Dict], correlation_id: str) -> Order:
        """Create a new order."""
        with order_service_log.with_correlation_id(correlation_id):
            ...
            order_service_log.info("Creating order",
                                 order_id=order_id,
                                 user_id=user_id,
            ...
            with order_service_log.time("order_creation", order_id=order_id):
                ...
                order_service_log.info("Order created successfully",
                                     order_id=order_id,
                                     user_id=user_id,
                                     total_amount=total_amount,
                                     item_count=len(items),
    ...
    async def update_order_status(self, order_id: int, status: str, correlation_id: str):
        """Update order status."""
        with order_service_log.with_correlation_id(correlation_id):
            if order_id in self.orders:
                ...
                order_service_log.info("Order status updated",
                                     order_id=order_id,
                                     old_status=old_status,
                                     new_status=status)
            else:
                order_service_log.error("Cannot update order: not found", order_id=order_id)
...
class PaymentService:
    ...
    async def process_payment(self, order: Order, correlation_id: str) -> bool:
        ...
        with payment_service_log.with_correlation_id(correlation_id):
            payment_service_log.info("Processing payment",
                                   order_id=order.id,
                                   amount=order.total_amount,
                                   user_id=order.user_id)
            
            with payment_service_log.time("payment_processing", 
                                        order_id=order.id,
                                        amount=order.total_amount):
                ...
                if success:
                    payment_service_log.info("Payment processed successfully",
                                           order_id=order.id,
                                           amount=order.total_amount,
                                           transaction_id=f"txn_{order.id}_{int(time.time())}")
                else:
                    payment_service_log.error("Payment processing failed",
                                            order_id=order.id,
                                            amount=order.total_amount,
...
class NotificationService:
    ...
    async def send_order_confirmation(self, user: User, order: Order, correlation_id: str):
        """Send order confirmation notification."""
        with notification_service_log.with_correlation_id(correlation_id):
            notification_service_log.info("Sending order confirmation",
                                        user_id=user.id,
                                        user_email=user.email,
                                        order_id=order.id,
                                        notification_type="order_confirmation")
            
            with notification_service_log.time("notification_send",
                                             notification_type="order_confirmation",
                                             user_id=user.id):
                # Simulate email sending
                await asyncio.sleep(0.05)
                
                notification_service_log.info("Order confirmation sent successfully",
                                            user_id=user.id,
                                            order_id=order.id,
                                            delivery_method="email")
    
    async def send_payment_failure_notification(self, user: User, order: Order, correlation_id: str):
        """Send payment failure notification."""
        with notification_service_log.with_correlation_id(correlation_id):
            notification_service_log.info("Sending payment failure notification",
                                        user_id=user.id,
                                        user_email=user.email,
                                        order_id=order.id,
            ...
            with notification_service_log.time("notification_send",
                ...
                notification_service_log.info("Payment failure notification sent",
                                            user_id=user.id,
                                            order_id=order.id,
                                            delivery_method="email")
...
class APIGateway:
    ...
    async def process_order_request(self, user_id: int, items: List[Dict], correlation_id: str) -> Dict:
        """Process a complete order request through the microservices."""
        with api_gateway_log.with_correlation_id(correlation_id):
            api_gateway_log.info("Order request received",
                               user_id=user_id,
                               item_count=len(items),
            ...
            try:
                with api_gateway_log.time("full_order_processing", user_id=user_id):
                    ...
                    if not user:
                        api_gateway_log.error("Order failed: user not found", user_id=user_id)
                        return {"success": False, "error": "User not found"}
                    
                    # Step 2: Create order
                    order = await self.order_service.create_order(user_id, items, correlation_id)
                    
                    # Step 3: Process payment
                    payment_success = await self.payment_service.process_payment(order, correlation_id)
                    
                    if payment_success:
                        # Step 4a: Update order status to completed
                        await self.order_service.update_order_status(order.id, "completed", correlation_id)
                        
                        # Step 5a: Send confirmation
                        await self.notification_service.send_order_confirmation(user, order, correlation_id)
                        ...
                        api_gateway_log.info("Order processed successfully",
                                           user_id=user_id,
                                           order_id=order.id,
                        ...
                        await self.order_service.update_order_status(order.id, "failed", correlation_id)
                        ...
                        api_gateway_log.error("Order failed due to payment issues",
                                            user_id=user_id,
                                            order_id=order.id,
                                            total_amount=order.total_amount)
                ...
                api_gateway_log.exception("Unexpected error during order processing",
                                        user_id=user_id,
...
async def simulate_load_test(gateway: APIGateway, num_requests: int = 10):
    ...
    api_gateway_log.info("Load test completed",
